...
In this project we explore the use of Twitter and news articles as alternative data sources to predict the stock values of leading green companies such as First Solar, Siemens, etc. We show our methods to acquire data from alternative sources such as Twitter and news articles and perform sentiment analysis over time. Using the time series obtained, we perform lag correlation and Granger causality, and TF-IDF analysis to investigate if information gained from the alternative data sources and the stock market prices of leading green energy companies are in any way correlated.
\hl{doublecheck}
\end{abstract}
...
% --------------------------------------------------------
\subsection{Description of Datasets} % data description report 
% --------------------------------------------------------
\subsubsection{Twitter Data}
...
\begin{enumerate}
    \item Remove punctuation - Remove all punctuation from the sentences.
...
\subsubsection{Twitter and News Article Data}
Using the cleaned text data from above, for each entry in the Twitter and news article data we also created a field for sentiment. Using the NLTK Sentiment Intensity Analyzer, we found the sentiment for each body of text. These were used to construct time series in modeling.
...
\begin{enumerate}
    ...
    \item Compound Sentiment: Less than zero signifies negative sentiment and greater than zero signifies positive sentiment. 0 score value here corresponds to neutral sentiment
...
CoreNLP enables users to derive linguistic annotations for text. Although it is a very powerful tool, but following are a few issues that came-forth whiles using it which  made us choose and thus opt for the NLTK sentiment analysis and let go of this tool.
